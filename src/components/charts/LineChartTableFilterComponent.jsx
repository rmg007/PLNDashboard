// src/components/charts/LineChartTableFilterComponent.jsx

import React, { useState, useEffect, useMemo, useCallback, forwardRef, useImperativeHandle } from 'react';
import LineChartTableComponent from './LineChartTableComponent';

/**
 * A component that extends LineChartTableComponent with filtering capabilities.
 ...
 * @returns {React.Component} The LineChartTableFilterComponent
 */
const LineChartTableFilterComponent = forwardRef(({
  id,
  data = [],
  xField,
  yField,
  ...
  // Create a ref to the LineChartTableComponent
  ...
  const enhancedProps = useMemo(() => {
    ...
    if (filterDisplayMode === 'highlight' && selectedValues.size > 0) {
      props.getLineColor = (item, index) => {
        return selectedValues.has(item[xField]) 
          ? props.lineColor || '#3498db' // Full color for selected
          : 'rgba(52, 152, 219, 0.3)'; // Subdued color for unselected
      };
      
      props.getLineOpacity = (item, index) => {
        return selectedValues.has(item[xField]) ? 1 : 0.3;
      };
    }
  ...
  // Animation configuration for Plotly
...
export default LineChartTableFilterComponent;
